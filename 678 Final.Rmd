...
First I will make some EDAs to better understand the data and have a basic knowledge of it, then I will try to fit the model using the anylysis, and finally I will output the result and discuss the change of League of Legends in S12 from MSI till end. I will mainly focus on the importance of each position and how to balance the strength of all champions in the future by comparing my result and what Riot does in the coming version v13.1.
...
Then I tidy the data by changing those character variables to numeric ones, for example by deleting character"%" and making the variables show decimal numbers, and I deleted those champions that presence are too low. I use picks+bans>10 as a condition.
...
There are 16 variables but obviously not all of them should be considered in fitting the model. So we are going to analysis the data, especially focus on v12.8 and v12.12 because these three versions is the beginning, middle and end for S12. After making EDAs of v12.12, I can make a model and try to fit v12.8 in it.
...
These two plots show DPM,CSM,GPM are related and by calculating the two Pearson correlation are 0.85 and 0.83. It is reasonable since the more CS you get, the more gold you can earn and the more damage you can deal, maybe some of the support champions even have more money but all they do in fights is giving shield so they don't have much improvement in damage, but those champions are just a few from all the champions so it doesn't matter much.

## Model Fitting

While finding the correlation matrix, the picks and bans can be substitute by presence, and game length is obviously not related with win rate so it is omitted. I reserve all the other variables and make the matrix.
...
Using 0.7 as a limit, we can see that the conclusion that GPM,CSM,DPM are related is correct. Also,CSD.15,GD.15,XPD.15 are related. So I choose KDA, presence and CSD.15 as the variables used in the model.Here's the model I choose:
...
After making the model, let's take a look at the fixed effect of it.
...
We can see that CSD.15 seems has less effect on win rate, then we give out the random effects below, we can see that it still has a little impact on the win rate.

```{r echo=FALSE}
round(ranef(model)$position, digits = 2)[1:5, ]
```

We should know that the game's champion pick is as follows: ban 6 champions, pick 6 champions,ban 4 more and pick 4 more. So maybe there's some differences between first 6 bans, but they are mainly for 5 and 6, because the first 4 bans are for the ops in that version, but from my perspective, the first 6 ban can be considered the same importance. We can divide ban turn into two parts, first 6 as one group and 7-10 as another.
...
First we can check whether the model is fit using Residual Plot and QQ Plot.
...
We can see that for the residual plot, the mean of the residuals is really close to 0 and for the QQ plot, most of the points are on the line, which demonstrates that the model fits and the model checking is complete.

## Comparing Models

By using the result above and trying to fit the same model by using the data v12.8, we have the results as below and we can compare the results.
...
We can see that presence matters much more that it did in the newest version. I think there are two main reasons. First pro plays in version 12.8 only include MSI games which means there are only 11 teams instead of the world's teams. Also, in order to get the champion, most of the teams have no time to practice new champions and instead, they copy others bans and picks lead to much higher influence in presence. The second reason is that after the MSI, Riot weaken those champions which are chosen often and make the new version more balanced, so more champions can be chosen for the teams.

# V. Discussion

## Future Prospects

In this project, I used multilevel model to fit the data of pro play in v12.12 and compare the result with v12.8 and get the result exactly what Riot do in the past half year. So in the future, according to the model of v12.12, Riot should decrease the impact of JUG and find a way to make TOP join in fights in the first 15 minutes. v13.1 will come out in January 2023, but the information comes out that they really decrease the impact of JUG, and they make TOP tanker, I don't know whether it can successfully make TOP more powerful in the games, but we will see.

## Shortage

First, some champions can be played at two positions but in order to group, I always use the position that the champion appears most frequently, which may cause the data not so accurate. Second, the variables considered can choose more, for example win rate between champions, win rate by time and etc. Then we can better analysis the data and choose the better formation.
Last but not least, it's a game not only of the pros but also for ordinary people like me. By checking the data, I found that some champions are really strong in low rank can't even appear in the pro play. So Riot not only have to find a balance on matches, but also for all the players and it's a really tough task.
